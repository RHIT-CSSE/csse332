...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    ShmID = createSharedMemoryRegion(SHM_KEY);
    ...
    if (pid < 0) {
      ...
      sprintf(path, "./%s", child);
      execlp(path, child, NULL);
      exit(0);
    }
    ...
    printf("main has detached its shared memory region...\n");

    /* Remove shared memory region */
    removeSharedMemory(ShmID);
    printf("main has removed its shared memory region...\n");
