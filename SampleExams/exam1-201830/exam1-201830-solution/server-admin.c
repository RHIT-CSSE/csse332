...
admin should restart it.  It should restart it up to MAX_STARTS -1 times,
and then it should assume there is a persistant problem and exit the
admin with an appropiate error message.
...
b) send a signal to kill the webserver itself
   ...
   command, so "kill(somePid, SIGKILL);" acts like running "kill somePid"
   on the command line.  Of course you'll need to know the
...
To test this code use the kill command like this in a second terminal 
(where adminpid is the pid of your admin process):
...
int childPid, numStarts;
int done = 0;

/* Forks a child process to run the web server application. 
 * The command name (e.g., ./server) and its arguments (as an
 * array of strings) are expected.
 */
void runWebServer() {
    ...
    for(int i = 0; i < MAX_STARTS; i++) {
        ...
        if ((childPid = fork()) == 0) {
            /* Can use execv(command, args) to make child run a 
               different program. */
            execv(command, args);
            exit(0);
        }
...
/* Catches SIGUSR1 signal and processes it */
...
int main(int argc, char *argv[]) {
    /* set up our signal handler to catch SIGUSR1 */
    int answer;
