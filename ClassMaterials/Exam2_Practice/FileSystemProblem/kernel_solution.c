...
void readFile(char *name, char *buffer)
{
    ...
    /* if the extra file entry is found */
    if(indir_entry >= 0){
        j = indir_entry;
        ...
        for (i=1; i<31; i++) {
            ...
            /* STEP 2 */
            itoa(directory[j*32+i],print_buff);
            printString(print_buff);

            readSector(fileChunk, directory[j*32+i]);
            for (k=0; k<SECTOR_SIZE; k++) {
                buffer[bufferPos] = fileChunk[k];
                bufferPos++;
            }
...
void writeFile(char *name, char *buffer, int numberOfSectors)
{
	...
	if (numberOfSectors > entrySize-nameLength+32-1) {
		printString(error);
		printNewLine();
		return;
	}
	...
	for (j=0, k=3; j<numberOfSectors; i++, j++) {
		...
		map[k] = 0xFF;
        if(j == 25){
            for(indir_entry = 0;indir_entry<512;indir_entry+=32){
                if(directory[indir_entry] == 0)
                    break;
            }
            if(indir_entry == 512){
                printString(error);
            ...
            directory[entryPos+i] = indir_entry/32;
            directory[indir_entry] = 0x01;
	...
	for (i=6, j=0; j<numberOfSectors; i++, j++) {
        ...
        if(j<=25 && indir_entry == 0){
            if(j == 25){
                indir_entry = directory[entryPos+i]*32;
                j--;
            }else
                writeSector(fileChunk, directory[entryPos+i]);
        }
        else{
            writeSector(fileChunk, directory[indir_entry+j-24]);
        }
	}
