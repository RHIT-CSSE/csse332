...
double number_sum(struct Number* num_array, int size) {
    ...
    for(int i = 0; i < size; i++) {
        ...
        if(current->type == FRACTION_TYPE) {
            struct Fraction* val = current->value;
            sum += (double) val->numerator/(double) val->denominator;
        }
    }
    return sum;
}
...
void allocate_double(struct Number* num_to_fill, double value) {
    ...
    if(new_double == NULL) exit(77);
    *new_double = value;
    num_to_fill->value = new_double;
    num_to_fill->type = DOUBLE_TYPE;
    num_to_fill->to_double = to_double_double;
}

void allocate_fraction(struct Number* num_to_fill, int numerator, int denominator) {
    struct Fraction *new_fract = malloc(sizeof(struct Fraction));
    if(new_fract == NULL) exit(78);
    new_fract->numerator = numerator;
    new_fract->denominator = denominator;
    num_to_fill->value = new_fract;
    num_to_fill->type = FRACTION_TYPE;
    num_to_fill->to_double = to_double_fraction;
}

void free_allocation(struct Number* num_to_free) {
    free(num_to_free->value);
    num_to_free->value = NULL; // for safety
}
...
hetergenous collection without modifying many parts of the code.
