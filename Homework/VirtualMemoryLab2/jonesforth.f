...
\ for contexts that support it, we can yield to host
: YIELD 1 PAUSE_WITH_CODE ;

\ flush forces output to be dealt with by host
: FLUSH 4 PAUSE_WITH_CODE ;

\ fork forks a child process
: FORK 5 PAUSE_WITH_CODE ;

\ CR prints a carriage return and also forces a flush
: CR '\n' EMIT FLUSH ;
...
( ABS puts the absolute value of a number on the stack )
...
(       TELL prints a string that's on the top of the stack.  It used to be
        defined in assembly but I've moved into pure FORTH because EMIT is
        buffered
)

: TELL DUP BEGIN 0<> WHILE SWAP DUP C@ EMIT 1+ SWAP 1- DUP REPEAT 2DROP ;
...
actually commenting this one out because ;CODE doesn't work well with garbage collection activitye
