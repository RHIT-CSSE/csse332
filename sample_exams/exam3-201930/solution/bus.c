...
int current_bus_capacity;
int total_boarded;
sem_t bus_leave;
sem_t bus_loading;
sem_t bus_at_destination;
...
void* passenger_thread(void *num_p) {
    ...
    sem_wait(&bus_loading);
    printf("Passenger %d boarding the bus\n", num);
    current_bus_capacity--;
    total_boarded++;
    if(current_bus_capacity == 0 || total_boarded == NUM_PASSENGERS) {
        sem_post(&bus_leave);
    } else {
        sem_post(&bus_loading);
    }
    sem_wait(&bus_at_destination);
...
void* bus_thread(void* ignored) {
    ...
    while(1) {
        ...
        current_bus_capacity = MAX_SEATS;
        sem_post(&bus_loading);
        sem_wait(&bus_leave);
...
int main(int argc, char **argv) {
    ...
    sem_init(&bus_leave, 0, 0);
    sem_init(&bus_loading, 0, 0);
    sem_init(&bus_at_destination, 0, 0);
