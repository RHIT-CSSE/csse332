...
char* thingToPrint;
sem_t printersWaiting, printerReadFinish;

void *printer(void *arg) {
  char* currentJob;
  while(1) {
    sem_wait(&printersWaiting);
    currentJob = thingToPrint;
    sem_post(&printerReadFinish);
    printf("Printing %s \n", currentJob);
    sleep(1);
...
void *application(void *arg) {
  ...
  for(i = 0; i < 5; i++) {
    ...
    thingToPrint = jobs[i];
    sem_post(&printersWaiting);
    sem_wait(&printerReadFinish);
  }
...
int main(int argc, char **argv) {
  ...
  sem_init(&printersWaiting, 0, 0);
  sem_init(&printerReadFinish, 0, 0);
