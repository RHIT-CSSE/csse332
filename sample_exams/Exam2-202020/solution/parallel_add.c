...
sem_t total_mutex, max_threads;
u_int32_t total_sum = 0;

void* slow_sum(u_int8_t data[]) {
...
int main(int argc, char** argv) {
    ...
    u_int8_t *chunk_and_size = malloc(chunk_size + 1);
    if(chunk_and_size == NULL) {
    ...
    pthread_t tids[MAX_FILE_SIZE / chunk_size + 1];
    sem_init(&total_mutex, 0, 1);
    sem_init(&max_threads,0, MAX_THREADS);
    ...
    while((read_result = read(read_fd, chunk_and_size + 1, chunk_size))) {
        ...
        sem_wait(&max_threads);
        pthread_create(&tids[num_threads], NULL, (void * (*)(void *)) slow_sum, chunk_and_size);
        num_threads++;
        
        chunk_and_size = malloc(chunk_size + 1);
    }
    free(chunk_and_size);
